...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (iMsg)
    {
        case WM_CREATE:
            ...
            &szFunctionOne, 1, GetUserDefaultLCID(), &dispID);

            if (FAILED(hr))
		    {
			    MessageBox(NULL, TEXT("Can Not Get ID Of Names"), TEXT("ERROR"), MB_OK | MB_ICONERROR | MB_TOPMOST);
			    pIDispatch->lpVtbl->Release(pIDispatch);
			    DestroyWindow(hwnd);
    		}
            hr = pIDispatch->lpVtbl->Invoke(pIDispatch, dispID, &IID_NULL, 
            GetUserDefaultLCID(), DISPATCH_METHOD, &params, &vRet, NULL, NULL);
            ...
            wsprintf(str, TEXT("Sum of %d and %d = %d"), num1, num2, vRet.lVal);
            MessageBox(hwnd, str, TEXT("Sum"), MB_OK);

            //Subtraction
            hr = pIDispatch->lpVtbl->GetIDsOfNames(pIDispatch, &IID_NULL, &szFunctionTwo, 1, GetUserDefaultLCID(), 
                &dispID);
		    ...
		    wsprintf(str, TEXT("Substraction of %d and %d = %d"), num1, num2, vRet.lVal);
