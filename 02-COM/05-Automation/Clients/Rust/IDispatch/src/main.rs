...
mod win32 {
	use crate::types::IID_IDISPATCH;
use libc::c_void;
	use crate::types::{DWORD, HRESULT, REFCLSID, REFIID, VARIANT};
	use crate::types::{IUnknown, IDispatch};
	...
	pub fn run_imymath_methods(num1: i32, num2: i32) {
		...
		*/
		let mut obj: *mut IDispatch = null_mut();
        ...
        VariantInit(&mut vArgs[0]);
        VariantInit(&mut vArgs[1]);

		match unsafe { CoCreateInstance(&CLSID_MY_MATH, null_mut(), CLSCTX_INPROC_SERVER, 
            &IID_IDISPATCH, transmute(&mut obj)) } {
			S_OK => (),
